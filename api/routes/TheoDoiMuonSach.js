...
router.get("/count/:SachId/", async (req, res) => {
  try {
    ...
    const countMuon = await TheoDoiMuonSach.countDocuments({ maSach: sachId });

    const daTra = await TheoDoiMuonSach.countDocuments({tinhTrang: true})

    // Tính toán số lượng sách còn lại
    const count = soQuyen - countMuon + daTra;
